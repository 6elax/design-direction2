...
3. **Command: /resolved**: If the user types `/resolved`, the agent must:
   ...
   - If the block is valid (at least 2 exchanges):
     ...
     - **MANDATORY Topic/Key Naming Rule**: The struggle key MUST be formatted as `author-descriptive-slug` using a short 3–5 word summary of the roadblock (e.g., `alexis-validation-gating`). It MUST NOT contain timestamps or random numeric strings (no `author-struggle-1234567890`), unless a number is directly part of the problem context (e.g., `mvp-2`).
     - **MANDATORY Summary Formatting Rule**: Run NLU synthesis to generate a SHORT paragraph summary written in plain, generalized language that readers from any other discipline/field can easily understand (no domain or implementation jargon), ending with exactly one Socratic pivot question on a new line.
     ...
     - **MANDATORY Dialogue Context Rule**: The `## 🎙️ Verbatim Dialogue History` section MUST include the conversational turns leading up to the roadblock BEFORE the user typed `/stuck` (at least 2–4 preceding turns detailing what led to the struggle), through the final resolution turns, strictly alternating `**User**:` and `> **Agent**:`. Overwrite the `## 🎙️ Verbatim Dialogue History` section in `.t4g/skill-weave/pending_struggle_log.md` with this complete context.
...
## Trigger the following at the end of each turn
- **Agent reflections**: At the end of every turn that involves direct user interaction or changes to the workspace (and NOT on turns that are purely meta-turns like status updates, reporting reflections, or asking for feedback on logs), you MUST dispatch a subagent to activate the /agent-reflections skill before you yield your turn and respond to the user. The new subagent should have the full context of the prior conversation.
