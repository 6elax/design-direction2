...
const CASES = [
  ...
  },
  {
    ...
    type: "DESIGN-FRICTION",
    author: "Varia",
    chatId: "D2oFz5XX0MLSOyqelYwS",
    phase_index: 0,
    start_message_index: 8,
    end_message_index: 11,
  ...
  },
  {
    ...
    chatId: "D2oFz5XX0MLSOyqelYwS",
    phase_index: 0,
    start_message_index: 24,
    end_message_index: 27,
  ...
  },
  {
    ...
    resolution: "Instead of choosing a single format, Alexis split the interface: a command-line utility for background logging and a code editor sidebar widget for interactive chat."
  },
  {
...
async function seed() {
  ...
  for (let i = 0; i < CASES.length; i++) {
    ...
    const conversationId = item.chatId || "00000000-0000-0000-0000-000000000000";
    const phaseIndex = item.phase_index !== undefined ? item.phase_index : 0;
    const startMsgIndex = item.start_message_index !== undefined ? item.start_message_index : 0;
    const endMsgIndex = item.end_message_index !== undefined ? item.end_message_index : 0;
    
    console.log(`[${i + 1}/${CASES.length}] Writing manual indices for ${key}...`);
    console.log(`  └─ Chat: ${conversationId} | Range: Phase ${phaseIndex}, [${startMsgIndex} to ${endMsgIndex}]`);
