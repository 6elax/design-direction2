...
2. **NLU Synthesis**: Clicking **Proceed** triggers the agent to synthesize the conversation transcript and write it to `.t4g/skill-weave/pending_struggle_log.md` (which is added to `.gitignore`). The review card must follow these strict formatting guidelines:
   *   **Invisible Metadata Block**: Technical database coordinates (`author`, `type`, `key`, `conversation_id`, `phase_index`, `start_message_index`, `end_message_index`, `project`) are stored in an HTML comment block (`<!-- METADATA: ... -->`) at the top of the file. This keeps the markdown preview clean and uncluttered for the user while enabling the parser script to extract exact metadata for Firestore.
   ...
   *   **Full Markdown Preservation**: All formatting elements (`code blocks`, `**bold**`, `*italics*`, `diffs`, newlines, list items, and `> blockquotes`) across `summary`, `roadblock`, `resolution`, and `dialogue_history` are 100% preserved in the cloud database.
   *   **Clean Dialogue History**: Must consist strictly of alternating User (`**User**:`) and Agent (`> **Agent**:`) turns, with formatting (newlines/indentation) preserved. All system messages, tool execution logs, compiler logs, and command outputs must be excluded.
