...
2. **NLU Synthesis**: Clicking **Proceed** triggers the agent to synthesize the conversation transcript and write it to `.t4g/skill-weave/pending_struggle_log.md` (which is added to `.gitignore`). The review card must follow these STRICT MANDATORY RULES:
   *   **MANDATORY Topic/Key Naming Rule**: The struggle key MUST be formatted as `author-descriptive-slug` using a short 3–5 word summary of the conceptual roadblock (e.g., `alexis-validation-gating`). It MUST NOT contain timestamps or random numeric strings (no `author-struggle-1234567890`), unless a number is directly part of the problem context (e.g., `mvp-2`).
   *   **MANDATORY Summary Rule**: The `Summary` field MUST BE A SHORT, GENERALIZED PARAGRAPH describing what happened in plain English so that readers from any other discipline can easily understand it (no domain or implementation jargon), ending with exactly one Socratic pivot question on a new line.
   *   **MANDATORY Pre-`/stuck` Dialogue Context Rule**: The `## 🎙️ Verbatim Dialogue History` section MUST INCLUDE THE CONVERSATIONAL CONTEXT BEFORE THE USER TYPES `/stuck` (at least 2–4 preceding turns detailing what led to the roadblock), through the final resolution turns. It must consist strictly of alternating User (`**User**:`) and Agent (`> **Agent**:`) turns with formatting preserved, completely excluding system logs, compiler dumps, and tool outputs.
   *   **Roadblock & Resolution Depth & Balance**: The `Roadblock` and `Resolution` sections should be as long and thorough as needed (spanning multiple paragraphs or bullet points if necessary) to convey all relevant information. Maintain high readability by including technical terms whenever directly relevant to the issue, while avoiding excessive or unnecessary jargon.
   *   **Invisible Metadata Block**: Technical database coordinates (`author`, `type`, `key`, `conversation_id`, `phase_index`, `start_message_index`, `end_message_index`, `project`) are stored in an HTML comment block (`<!-- METADATA: ... -->`) at the top of the file so the markdown preview remains clean.
   *   **Comparative Diffs & Resolution Mapping**: For technical/coding struggles involving code changes, comparative diffs illustrate the before-and-after fix and are saved under the `resolution` field in Firestore. For conceptual, design, or planning struggles without code changes, this section is omitted and the conceptual resolution is preserved directly.
   *   **Full Markdown Preservation**: All formatting elements (`code blocks`, `**bold**`, `*italics*`, `diffs`, newlines, list items, and `> blockquotes`) across all fields are 100% preserved when uploaded to Firestore.
